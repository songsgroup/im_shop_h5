...
export const formatMessageTime = (timestemp, keepSameYear = false) => {
  ...
  const keepYear = keepSameYear || !isThisYear(timestemp);
	...
	const locales = uni.getStorageSync('locale');
	if (!locales) {
...
export const sec2Time = (seconds) => {
  ...
  var result = "";
  if (seconds > 0) {
    result = "" + parseInt(seconds) + `[${i18n.t('秒')}]`;
  }
  if (theTime1 > 0) {
    result = "" + parseInt(theTime1) + `[${i18n.t('分钟')}]` + result;
  }
  if (theTime2 > 0) {
    result = "" + parseInt(theTime2) + `[${i18n.t('小时')}]` + result;
  }
  if (theTime3 > 0) {
    result = "" + parseInt(theTime3) + `[${i18n.t('天')}]` + result;
  }
  return result;
};
...
export const parseMessageByType = (pmsg) => {
  ...
  switch (pmsg.contentType) {
    ...
    case MessageType.RevokeMessage:
      ...
      const revoker = isSelf(data.revokerID) ? `${i18n.t('你')}` : data.revokerNickname;
      ...
      if (isAdminRevoke) {
        return `${revoker}${i18n.t('撤回了一条')}${sourcer}的消息`;
      }
      return `${revoker}${i18n.t('撤回了一条消息')}`;
    case MessageType.CustomMessage:
			if (pmsg.customElem.description === 'AUDIO') {
				return `[${i18n.t('语音通话')}]`;
			} else if (pmsg.customElem.description === 'VIDEO') {
				return `[${i18n.t('视频通话')}]`;
			} else if (pmsg.customElem.extension === 'Robot-Customer-Service') {
				return `[${i18n.t('机器人回复')}]`;
			}	else if (pmsg.customElem.extension === 'REDPACKET') {
				return `[${i18n.t('红包')}]`;
			} else if (pmsg.customElem.extension === 'TRANSFER') {
				return `[${i18n.t('转账')}]`;
			} else if (pmsg.customElem.extension === 'NOTICE_MSG') {
				return `[${i18n.t('通知消息')}]`;
			} else if (pmsg.customElem.extension === 'SHARE-MESSAGE') {
				return `[${i18n.t('分享')}]`;
			} else if (pmsg.customElem.extension === 'NOFRIEND') {
				return `[${i18n.t('对方已将你删除')}]`;
			} else {
    ...
    case MessageType.MemberInvited:
      ...
      return `${getName(inviteOpUser)}${i18n.t('邀请了')}${inviteStr}${
    ...
    case MessageType.GroupInfoUpdated:
      ...
      let updateFiled = `${i18n.t('群设置')}`;
      if (groupUpdateDetail.group.notification) {
        updateFiled = `${i18n.t('群公告')}`;
      }
      if (groupUpdateDetail.group.groupName) {
        updateFiled = `${i18n.t('群名称为')} ${groupUpdateDetail.group.groupName}`;
      }
      if (groupUpdateDetail.group.faceURL) {
        updateFiled = `${i18n.t('群头像')}`;
      }
      if (groupUpdateDetail.group.introduction) {
        updateFiled = `${i18n.t('群介绍')}`;
      }
      return `${getName(groupUpdateUser)}${i18n.t('修改了')}${updateFiled}`;
    case MessageType.GroupOwnerTransferred:
      ...
      return `${getName(transferOpUser)}${i18n.t('将群主转让给')}$${getName(newOwner)}`;
    case MessageType.GroupDismissed:
      ...
      return `${getName(dismissUser)}${i18n.t('解散了群聊')}$`;
    case MessageType.GroupMuted:
      ...
      return `${getName(groupMuteOpUser)}${i18n.t('开启了全体禁言')}$`;
    case MessageType.GroupCancelMuted:
      ...
      return `${getName(groupCancelMuteOpUser)}${i18n.t('取消了全体禁言')}$`;
    case MessageType.GroupMemberMuted:
      ...
      return `${getName(gmMutedDetails.opUser)}${i18n.t('禁言了')}${getName(
    ...
    case MessageType.GroupMemberCancelMuted:
      ...
      return `${getName(gmcMutedDetails.opUser)}${i18n.t('取消了禁言')}${getName(
    ...
    case MessageType.GroupAnnouncementUpdated:
      ...
      return `${getName(groupAnnouncementUpdateUser)}${i18n.t('修改了群公告')}`;
    case MessageType.GroupNameUpdated:
      ...
      return `${getName(groupNameUpdateUser)}${i18n.t('修改了群名称为')}${
    ...
    case MessageType.BurnMessageChange:
      ...
      return `阅后即焚已${burnDetails.isPrivate ? `${i18n.t('开启')}` : `${i18n.t('关闭')}`}`;
    default:
      return "";
  }
};
...
export const formatConversionTime = (timestemp) => {
  ...
  if (fromNowStr.includes(`${i18n.t('秒')}`)) {
    return `${i18n.t('刚刚')}`;
  }

  if (!fromNowStr.includes(`${i18n.t('秒')}`) && !fromNowStr.includes(`${i18n.t('分钟')}`)) {
    return dayjs(timestemp).calendar();
  }
...
export const tipMessaggeFormat = (msg, currentUserID) => {
  if (msg.contentType === MessageType.RevokeMessage) {
    ...
    revoker = currentUserID === data.revokerID ? `${i18n.t('你')}` : data.revokerNickname;
    ...
    if (isAdminRevoke) {
      return `${revoker}${i18n.t('撤回了一条')}${sourcer}${i18n.t('的消息')}`;
    }
    return `${revoker}${i18n.t('撤回了一条消息')}`;
  }

  const getName = (user) =>
    user.userID === currentUserID ? `${i18n.t('你')}` : user.nickname;
  ...
  switch (msg.contentType) {
    case MessageType.FriendAdded:
      return `${i18n.t('你们已经是好友了')}~`;
    case MessageType.GroupCreated:
      const groupCreatedDetail = JSON.parse(msg.notificationElem.detail);
      const groupCreatedUser = groupCreatedDetail.opUser;
      return `${parseInfo(groupCreatedUser)}${i18n.t('创建了群聊')}`;
    case MessageType.GroupInfoUpdated:
      ...
      return `${parseInfo(groupUpdateUser)}${i18n.t('修改了')}${updateFiled}`;
    case MessageType.GroupOwnerTransferred:
      const transferDetails = JSON.parse(msg.notificationElem.detail);
      const transferOpUser = transferDetails.opUser;
      const newOwner = transferDetails.newGroupOwner;
      return `${parseInfo(transferOpUser)}${i18n.t('转让群主给')}${parseInfo(newOwner)}`;
    case MessageType.MemberQuit:
      const quitDetails = JSON.parse(msg.notificationElem.detail);
      const quitUser = quitDetails.quitUser;
      return `${parseInfo(quitUser)}${i18n.t('退出了群组')}`;
    case MessageType.MemberInvited:
      ...
      inviteStr = inviteStr.slice(0, -1);
      return `${parseInfo(inviteOpUser)} ${i18n.t('邀请了')}${inviteStr}${
        invitedUserList.length > 3 ? "..." : ""
      }${i18n.t('加入群聊')}`;
    case MessageType.MemberKicked:
...
export const getDesignatedUserOnlineState = (userID, targetID) => {
  return new Promise(async (resolve, reject) => {
    ...
    // );
    const onlineStr = status ? `${i18n.t('在线')}` : `${i18n.t('离线')}`;
    resolve(onlineStr);
  });
};
